...
class Board {
    ...
    resetForAnalysis(flagIsMine, findObviousMines) {
        ...
        for (let i = 0; i < this.tiles.length; i++) {
            ...
            if (tile.isCovered()) {
                if (this.isTileAMine(tile)) {
                    // a tile is an obvious mine if every adjacent witness agrees it is
                    tile.setFoundBomb()
                }
    ...
    isTileAMine(tile) {
        ...
        if (witnessCount == 0) {
            ...
            // all the adjacent witnesses agree that this tile is a mine
            return true
        }
    }
    ...
    checkWitness(tile) {
        ...
        if (coveredCount > 0 && tile.getValue() == coveredCount) {
            // only room left for mines
            return true
        }
    }
    ...
    getHashValue() {
        let hash =
            (31 * 31 * 31 * this.num_bombs + 31 * 31 * this.getFlagsPlaced() + 31 * this.width + this.height) % this.MAX
    ...
    getSimpleCompressedData() {
        ...
        for (let y = 0; y < height; y++) {
            ...
            for (let x = 0; x < width; x++) {
                ...
                if (tile.isFlagged()) {
                    ...
                    // a covered tile, or an exploded mine
                    xa.push(9) // hidden
                } else {
        ...
        for (let y = 0; y < height; y++) {
            for (let x = 0; x < width; x++) {
                ...
                if (value < 9) {
                    ...
                    if (spaces == value) {
                        // if the number of spaces equals the value then anything unflagged is a 'hidden flag'
        ...
        for (let y = 0; y < height; y++) {
            for (let x = 0; x < width; x++) {
                ...
                if (value == 10 || value == 11) {
                    ...
                    if (okay) {
                        // every adjacent revealed tile can be reduced by 1
                        for (let y1 = ys; y1 <= ye; y1++) {
                            for (let x1 = xs; x1 <= xe; x1++) {
                                if (a[y1][x1] < 9) {
                                    a[y1][x1]--
                        ...
                        if (a[y][x] == 10) {
                            // placed flag that overflags a revealed tile
                            a[y][x] = 12 // overflagged ==> flag that isn't inflated
                        } else if (a[y][x] == 11) {
                            // hidden flag that overflags a revealed tile
        ...
        for (let y = 0; y < height; y++) {
            for (let x = 0; x < width; x++) {
                ...
                if (value == 10) {
                    // flagged and deflated
                    data = data + "F"
                } else if (value == 11) {
                    // a mine is here but not flagged, deflated
                    data = data + "I"
                } else if (value == 12) {
                    // a flag which wasn't deflated
                    data = data + "O"
                } else if (value == 9) {
                    // a hidden tile
                    data = data + "H"
                } else {
                    // a revealed tile's value (after deflation)
                    data = data + value
                }
            }
        }
